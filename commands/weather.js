	...
	try {
		...
		return (
			new MessageEmbed()
				...
				.addField(
					'Wind',
					`${json.wind.speed} m/s, from the ${json.wind.deg}`,
					true
				)
				.addField(
					'Country',
					`:flag_${json.sys.country.toLowerCase()}:`,
					true
				)
				.addField(
					'Temperature',
				...
				.addField(
					'Feels like',
					`${Math.round(json.main.feels_like - 273.15)} °C`,
					true
				)
...
weather.usage = `<city>`;
weather.shortDesc = 'Returns the weather.';
weather.args = false;
weather.aliases = ['w'];
