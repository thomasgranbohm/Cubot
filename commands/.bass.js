...
module.exports = {
	...
	execute(message, args) {
		let { connection } = message.member.voice
		if (connection && connection.dispatcher && connection.dispatcher.speaking) {
			console.log("bassboosting")
			let { stream, transcoder, encoder } = message.client.playing.get(message.member.voice.guild.id);
			...
			message.client.playing.get(message.member.voice.guild.id).stream.pipe(otherTranscoder);
			...
			otherTranscoder.once('data', () => {
				...
				encoder.once('data', (data) => {
					console.log(data)
					message.member.voice.connection.playOpusStream(message.client.playing.get(message.member.voice.guild.id).stream)
				})
			})
		} else {
