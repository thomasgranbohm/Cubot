...
export default async function (client: Bot, guildId: string, equalizer: Equalizer) {
	...
	const newEqualizer = currentEqualizer !== equalizer ? equalizer : flat;
	await player.equalizer(newEqualizer.bands);

	client.servers.set(guildId, { ...server, equalizer: newEqualizer });

	return new MessageEmbed().setTitle(`Changing equalizer to ${newEqualizer.name}...`);
}
