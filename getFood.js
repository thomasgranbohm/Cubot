	...
	form: {
		...
		request.post(options).on('response', (response) => {
			...
			request.post(options, (err, httpResponse, body) => {
				...
				let lunches = table.split('<td valign="top" class="printSmall">').slice(1).map(obj => obj.split("</td")[0].replace(new RegExp("<br /><br />", 'g'), "\n").replace(/ +(?= )/g, '').replace(new RegExp("<br />", 'g'), ""))
				...
				fs.writeFile('/home/thomas/CuBot/utils/lunch.json', JSON.stringify(toJSON, null, 4), (err) => {
					...
					if (toJSON.length == 0) console.log("The Array is empty.");
